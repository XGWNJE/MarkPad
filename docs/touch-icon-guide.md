...
已显示的网站图标也能从卡片右键菜单进入相同的背景编辑器。编辑过程会临时同步到原卡片，取消即恢复；该背景按书签单独保存，不会将网站图标变成自定义图标。网站图标刷新或资源更新时，自动融合会重新分析。
...
内置图标自动匹配不读取 URL、域名或路径，也不显示首字母、emoji 或通用占位图标。网站图标只在卡片进入可见区域后按需解析，识别 `icon`、`shortcut icon`、`apple-touch-icon`、Manifest 图标和同源 `/favicon.ico`；不在启动时批量请求。网站图标按完整书签 URL 缓存，成功缓存 30 天、失败缓存 24 小时。
...
- 位图上传使用原始解码尺寸校验，宽高都不得低于 256px，且文件不超过 1MB。
